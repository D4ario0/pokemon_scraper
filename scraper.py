...
# Given pokemondb.net robots.txt policy of 4 seconds delay, requests will be delayed.
# Accidentally the scraper was tested against a no time restriction and the ~1_000 request were completed in roughly 2 seconds
# The scrape time for 1025 pokemon entries with a 4 second delay per request is ~1h10m, leave the script running in the background
async def main():
    """Main function to generate URLs, fetch Pokémon data, and save it to a JSON file."""
    urls = URLGenerator("poke_list.csv").generate()
    db = TinyDB("pokemonDB.json")
    pokemon_table = start_db(db)
